...
def complexity_conv2d(cx, w_in, w_out, k, stride, padding, groups=1, bias=False):
    ...
    k_h = k if type(k) is int else k[0]
    k_w = k if type(k) is int else k[1]
    stride_h = stride if type(stride) is int else stride[0]
    stride_w = stride if type(stride) is int else stride[1]
    padding_h = padding if type(padding) is int else padding[0]
    padding_w = padding if type(padding) is int else padding[1]
